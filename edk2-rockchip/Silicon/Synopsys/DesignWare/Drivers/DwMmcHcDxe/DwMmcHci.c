  ...
  )
{
  ...
  do {
    ...
    ClkEna = 1;
    MmioWrite32 (DevBase + DW_MMC_CLKENA, ClkEna);
    MmioWrite32 (DevBase + DW_MMC_CLKSRC, 0);
  ...
  )
{
  ...
  //
  // On Rockchip SoCs, after reset leave CTRL at 0 (RK3399 DwEmmcDxe pattern).
  // IDMAC_EN and DMA_EN are set only when DMA is started in DwMmcHcStartDma.
  // INT_EN, TMOUT, INTMASK etc. are configured by DwMmcHcReset() later.
  //
  // CTRL is left at its post-reset value (0x00000000).
  //

  //
  // Reset the IDMAC controller via BMOD (U-Boot / RK3399 DwEmmcDxe pattern).
  //
  MmioWrite32 (DevBase + DW_MMC_BMOD, DW_MMC_IDMAC_SWRESET);
  do {
    Status = MmioRead32 (DevBase + DW_MMC_BMOD);
  } while (Status & DW_MMC_IDMAC_SWRESET);
