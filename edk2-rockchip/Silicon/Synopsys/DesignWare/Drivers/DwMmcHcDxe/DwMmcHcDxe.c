  ...
  )
{
  ...
  //
  // Platform drivers (RkSdmmcDxe, RkEmmcDxe) may have already created
  // NonDiscoverableDevice handles before we loaded.  Scan for them and
  // trigger binding explicitly so both SD and eMMC get connected.
  //
  HandleBuffer = NULL;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "DwMmcHc Start: PlatformDwMmc not on Controller %r\n", Status));
    return Status;
  }
